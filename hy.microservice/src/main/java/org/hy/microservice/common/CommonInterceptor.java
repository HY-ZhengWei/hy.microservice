...
public class CommonInterceptor implements HandlerInterceptor
{
    ...
    private void attackJSSendAlarm(HttpServletRequest i_Request ,String i_ParamInfo)
    {
        ...
        $Logger.warn(XJava.getParam("MS_Common_ServiceName").getValue() + "：注入攻击：" + v_Msg);
    }
    ...
    private void corsSendAlarm(HttpServletRequest i_Request ,String i_ParamInfo)
    {
        ...
        String v_Msg = v_UserName + "(" + v_UserCode + ") 请求页面：" + i_Request.getRequestURL().toString() + ":" + Help.NVL(i_Request.getQueryString()) + ":" + i_ParamInfo + "。" + Date.getNowTime().getFull();
        $Logger.warn(XJava.getParam("MS_Common_ServiceName").getValue() + "：非法跨域：" + v_Msg);
    }
...
//            }
    ...
    {
        Map<String ,Param> v_DNSMap = (Map<String ,Param>)XJava.getObject("MS_Common_DNSConfigs");
        if ( Help.isNull(v_DNSMap) )
        {
            return true;
        }
        else
    ...
    {
        List<Param> v_DNSIPList = (List<Param>)XJava.getObject("MS_Common_DNSIPConfigs");
        if ( Help.isNull(v_DNSIPList) )
        {
            return true;
        }
        else
